...
pub enum MpcConfigIncompatibility {
    ...
    #[error("domain_id mismatch: expected {expected:?}, actual {actual:?}")]
    DomainIdMismatch {
        expected: DomainId,
        actual: DomainId,
    },
    ...
    #[error(
        "public_key shape mismatch for suite {suite:?}: expected {expected_bytes} bytes, actual {actual_bytes} bytes"
    )]
    PublicKeyShape {
        suite: MpcSuite,
