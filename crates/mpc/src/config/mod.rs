...
//!    KeyId, HPKE public key, control-plane algorithm identifiers, and
//!    approved Enclave Measurement that the host must trust before it does
//!    any of that work.
//! 2. Parsing the spec-shaped JSON payload the MPC endpoint serves.
//! 3. Validating the parsed config against the Coprocessor's
//!    [`MpcConfigExpectations`] (chain, domain, reader-key algorithm,
//!    ciphertext suite) so incompatible configuration is rejected before
//!    work is scheduled.
//!
...
//! expected vs actual algorithm identifiers). No public-key bytes, key
//! material, or plaintext appear in error values.
...
pub use config::{
    CiphertextSuite, MpcConfigExpectations, MpcPublicConfig, ReaderKeyAlgorithm, X25519PublicKey,
};
