...
`Symbolic-Execution/coprocessor`. Use the `gh` CLI for all operations.
...
- **Read an issue**: `gh issue view <number> --comments`, filtering comments by
  `jq` and also fetching labels when useful.
- **List issues**:
...
Create a GitHub issue in `Symbolic-Execution/coprocessor`.
